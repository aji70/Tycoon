...
function asPromoAddressSet(user) {
  const set = new Set();
...
async function verifyCollectiblePurchaseReceipt({ txHash, tokenId, recipient, chain }) {
...
export async function smartWalletBuyCollectible(req, res) {
  try {
    ...
    let bonus = { applied: false };
    if (isMinipayBogoPromoMode(req.body?.promoMode)) {
      bonus = await claimMinipayPerkBogo({
        claimKey: `tx:${hash}`,
        userId: user.id,
        tokenId: String(tokenId),
        chain,
        deliveryAddress: smartWallet,
        source: MINIPAY_BOGO_PROMO_MODE,
        purchaseTxHash: hash,
      });
    }
    return res.status(200).json({ success: true, data: { hash, bonus } });
  } catch (err) {
 ...
 * One-time backend bonus delivery for a verified MiniPay collectible purchase tx.
 * Body: { txHash: string, tokenId: string|number, recipient?: string, chain?: string, promoMode: "minipay_bogo" }
 */
export async function claimPerkBogo(req, res) {
  try {
    if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
    if (!isMinipayBogoPromoMode(req.body?.promoMode)) {
      return res.status(400).json({ success: false, message: "Promo mode not enabled" });
    }

    const txHash = String(req.body?.txHash || "").trim();
    const tokenId = String(req.body?.tokenId || "").trim();
    const chain = User.normalizeChain(req.body?.chain || req.user.chain || "CELO");
    if (!/^0x[a-fA-F0-9]{64}$/.test(txHash)) {
      return res.status(400).json({ success: false, message: "Valid txHash required" });
    }
    if (!/^\d+$/.test(tokenId) || BigInt(tokenId) <= 0n) {
      return res.status(400).json({ success: false, message: "Valid tokenId required" });
    }

    const allowed = asPromoAddressSet(req.user);
    ...
    logger.error({ err: err?.message, userId: req.user?.id }, "claimPerkBogo failed");
    return res.status(500).json({ success: false, message: err?.message || "Failed to claim promo perk" });
  }
}
