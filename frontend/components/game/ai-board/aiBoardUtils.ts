/**
 * Shared AI board helpers (buy score, etc.) used by useAIBoardLogic and both desktop/mobile AI boards.
 */
import type { Property, Player, GameProperty } from "@/types/game";
...
export function calculateBuyScore(
  ...
  if (group && !["railroad", "utility"].includes(property.color!)) {
    const owned = group.filter(
      (id) => gameProperties.find((gp) => gp.property_id === id)?.address === player.address
    ).length;
  ...
  if (property.color === "railroad") {
    const owned = gameProperties.filter(
      (gp) =>
