...
const GameSettings = () => {
  ...
  const checkRegistration = async () => {
    try {
      ...
      toast.error(err?.message || 'Failed to check registration status');
    }
  };
  ...
  const handleCreateGame = async () => {
    ...
    if (gameType !== '0' && gameType !== '1') {
      toast.error('Game type must be Public (0) or Private (1)');
      return;
    }
    if (isNaN(numPlayers) || numPlayers < 2 || numPlayers > 8) {
      toast.error('Number of players must be between 2 and 8');
      return;
    }
    ...
    try {
      ...
      toast.success('Game created successfully! Redirecting...');
          ...
          <div className="w-full max-w-[792px] bg-[#010F10] rounded-[12px] border-[1px] border-[#003B3E] md:p-[40px] p-[20px] flex flex-col gap-4">
            <div className="w-full flex justify-between items-center">
              ...
              >
                ...
                <SelectContent>
                  {tokens.map((token) => (
                    <SelectItem key={token.name} value={token.name} className="flex items-center gap-2">
